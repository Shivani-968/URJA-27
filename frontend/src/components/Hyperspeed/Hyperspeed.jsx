...
const Hyperspeed = ({
  effectOptions = {
    ...
    /* =====================================================
       URJA VORTEX COLOR THEME
       BLACK + NEON PINK + ELECTRIC BLUE + WHITE
       ===================================================== */
  ...
  useEffect(() => {
    ...
    const distortions = {
        ...
        getDistortion: `
          ...
          vec3 getDistortion(float progress){
            ...
            return vec3(
              ...
              cos(
                progress * PI * uFreq.x + uTime
              ) * uAmp.x
              -
              cos(
                movementProgressFix * PI * uFreq.x + uTime
              ) * uAmp.x,

              nsin(
                progress * PI * uFreq.y + uTime
              ) * uAmp.y
              -
              nsin(
                movementProgressFix * PI * uFreq.y + uTime
              ) * uAmp.y,

              nsin(
                progress * PI * uFreq.z + uTime
              ) * uAmp.z
              -
              nsin(
                movementProgressFix * PI * uFreq.z + uTime
              ) * uAmp.z
            );
          }
        `,
      ...
      xyDistortion: {
        ...
        getDistortion: `
          ...
          vec3 getDistortion(float progress){
            ...
            return vec3(
              ...
              -
              sin(
                movementProgressFix * PI * uFreq.y +
                PI/2. +
                uTime
              ) * uAmp.y,
        ...
        getJS: (progress, time) => {
          ...
          const lookAtAmp = new THREE.Vector3(
            2,
            0.4,
            1
          );
      ...
      LongRaceDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const uFreq =
            LongRaceUniforms.uFreq.value;

          const uAmp =
            LongRaceUniforms.uAmp.value;
      ...
      turbulentDistortion: {
        ...
        getDistortion: `
          ...
          vec3 getDistortion(float progress){
            ...
            return vec3(
              ...
              getDistortionX(progress)
              -
              getDistortionX(0.0125),

              getDistortionY(progress)
              -
              getDistortionY(0.0125),
        ...
        getJS: (progress, time) => {
          const uFreq =
            turbulentUniforms.uFreq.value;

          const uAmp =
            turbulentUniforms.uAmp.value;

          const getX = (p) =>
            Math.cos(
              Math.PI * p * uFreq.x + time
            ) *
              uAmp.x +
            Math.pow(
              Math.cos(
      ...
      deepDistortion: {
        ...
        getJS: (progress, time) => {
          const uFreq =
            deepUniforms.uFreq.value;

          const uAmp =
            deepUniforms.uAmp.value;

          const uPowY =
            deepUniforms.uPowY.value;

          const getX = (p) =>
            Math.sin(
              p * Math.PI * uFreq.x + time
            ) *
            uAmp.x;
    ...
    class CarLights {
      ...
      init() {
        ...
        instanced.instanceCount =
          options.lightPairsPerRoadWay * 2;
        ...
        for (
          let i = 0;
          i < options.lightPairsPerRoadWay;
          i++
        ) {
          const radius = random(
            options.carLightsRadius
          );

          const length = random(
            options.carLightsLength
          );
          ...
          const carWidth =
            random(
              options.carWidthPercentage
            ) * laneWidth;

          const carShiftX =
            random(options.carShiftX) *
            laneWidth;
    ...
    const roadMarkings_fragment = `
      ...
      float brokenLineWidth =
        laneWidth *
        uBrokenLinesWidthPercentage;

      float laneEmptySpace =
        1. -
        uBrokenLinesLengthPercentage;
      ...
      vec3 vortexLineColor;

      if (colorPosition < 0.3333) {

        vortexLineColor =
          uLinePink;

      } else if (colorPosition < 0.6666) {
    ...
    class App {
      ...
      initPasses() {
        ...
        const smaaPass =
          new EffectPass(
            ...
            new SMAAEffect({
              ...
              searchImage:
                SMAAEffect.searchImageDataURL,

              areaImage:
                SMAAEffect.areaImageDataURL,
            })
          );
        ...
        this.bloomPass.renderToScreen =
          false;

        smaaPass.renderToScreen =
          true;
      ...
      update(delta) {
        ...
        this.rightCarLights.update(
          time
        );

        this.leftCarLights.update(
          time
        );

        this.leftSticks.update(
          time
        );
